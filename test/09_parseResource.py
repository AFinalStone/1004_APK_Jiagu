        ...
        while True and self.axml.is_valid():
            ...
            if _type == START_DOCUMENT:
                ...
                self.buff += u'<' + self.getPrefix(
                    "" if self.axml.getPrefix() == self.axml.xmlns_android else self.axml.getPrefix(
                    )) + self.axml.getName() + u' '
                ...
                for i in range(0, self.axml.getAttributeCount()):
                    ...
                    if attrnamesys:
                        ...
                        attrname = "%s%s" % (
                            self.getPrefix(self.axml.getAttributePrefix(i)), self.axml.getAttributeName(i))
                    j = 0
                ...
                self.buff += "</%s%s>" % (
                    self.getPrefix("" if self.axml.getPrefix() == self.axml.xmlns_android else self.axml.getPrefix()),
                    self.axml.getName())
        ...
        for i in range(0, self.packageCount):
            ...
            while not self.buff.end():
                ...
                if header.type == RES_TABLE_TYPE_SPEC_TYPE:
                    ...
                    if a_res_type.config.size >= 52 and entries_0_and_1[:4] == entries_0_and_1[
                                                                               -4:] == "\x00\x00\x00\x00":
                        self.buff.read(4)
                    ...
                    if header.type == RES_TABLE_TYPE_TYPE:
                        ...
                        for entry, res_id in entries:
                            if entry != -1:
                                ...
                                self.id2value[package_name][ate.mResId][1][
                                    a_res_type.config.get_config_name()] = ate.get_key_data() if a_res_type.get_type() in {
                                    "string", "drawable", "mipmap"} else ate.get_value()
                                nb_i += 1
...
if __name__ == "__main__":
    # buf1 = (lambda f: (f.read(), f.close()))(open(r"resources.arsc", "rb"))[0]
    # dic2 = ARSCParser(buf1).get_dict()
    # print(dic2)
    buf1 = (lambda f: (f.read(), f.close()))(open(r"AndroidManifest.xml", "rb"))[0]
